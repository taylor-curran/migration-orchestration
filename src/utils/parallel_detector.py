...
class ParallelDetector:
    ...
    def get_execution_plan(self) -> ExecutionPlan:
        ...
        serial_tasks = [
            task["id"] for task in self.tasks if task["id"] not in parallel_task_ids
        ]

        # Calculate total serial duration
        total_serial = sum(task.get("estimated_hours", 8) for task in self.tasks)
        ...
        max_parallelism = max((group.size for group in parallel_groups), default=1)
...
if __name__ == "__main__":
    ...
    example_tasks = [
        ...
        {
            "id": "migrate_001",
            "title": "Migrate service A",
            "depends_on": ["setup_001", "setup_002", "validator_001"],
            "estimated_hours": 12,
        },
        {
            "id": "migrate_002",
            "title": "Migrate service B",
    ...
    for group in results["parallel_groups_detail"]:
        print(
            f"  Level {group['level']}: {', '.join(group['tasks'])} ({group['size']} tasks, saves {group['time_saved']}h)"
        )
